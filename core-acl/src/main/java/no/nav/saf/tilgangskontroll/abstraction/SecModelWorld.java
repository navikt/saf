...
public class SecModelWorld {
	...
	List<TilgangDokumentInfo> dokumentInfo;
	...
	public <T> void put(List<T> entiteter) {
		if (!entiteter.isEmpty()) {
			...
			} else if (checkType instanceof TilgangDokumentInfo) {
				this.dokumentInfo = (List<TilgangDokumentInfo>) entiteter;
			}
	...
	public void add(TilgangDokumentInfo dokumentInfo) {
		if (this.dokumentInfo == null) {
			this.dokumentInfo = Lists.newArrayList();
		}
		if (dokumentInfo != null) {
			this.dokumentInfo.add(dokumentInfo);
		}
	}
}
