package no.nav.saf.query.dokumentoversikt;

import no.nav.saf.anticorruptionlayer.joark.ArkivJournalpostMapper;
import no.nav.saf.anticorruptionlayer.joark.domain.JournalpostDtoMapper;
import no.nav.saf.anticorruptionlayer.joark.hentjournalsakinfo.dto.JournalpostDto;
import no.nav.saf.anticorruptionlayer.joark.safintern.journalpost.ArkivJournalpost;
import no.nav.saf.domain.visningsmodell.Journalpost;
import no.nav.saf.tilgangskontroll.SafRequestContext;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Component;

import java.util.List;
import java.util.Objects;
import java.util.stream.Stream;

@Component
public class DokumentoversiktVisningsmodellRepository {
	private final JournalpostDtoMapper journalpostDtoMapper;

	@Autowired
	public DokumentoversiktVisningsmodellRepository(JournalpostDtoMapper journalpostDtoMapper) {
		this.journalpostDtoMapper = journalpostDtoMapper;
	}

	public List<Journalpost> findJournalposter(final List<String> journalpostIds,
											   final SafRequestContext safRequestContext) {
		return findJournalposter(journalpostIds.stream(), safRequestContext).toList();
	}

	public Stream<Journalpost> findJournalposter(final Stream<String> journalpostIds,
											   final SafRequestContext safRequestContext) {
		return journalpostIds.map(journalpostId -> {
					JournalpostDto journalpostDto = safRequestContext.getRequestCache().getJournalpost(journalpostId);
					return journalpostDtoMapper.mapJournalpostDto(journalpostDto, safRequestContext.getRequestCache());
				}).filter(Objects::nonNull);
	}

	public Stream<Journalpost> findJournalposterCurrent(final Stream<String> journalpostIds,
												 final SafRequestContext safRequestContext) {
		return journalpostIds.map(journalpostId -> {
			ArkivJournalpost arkivJournalpost = safRequestContext.getRequestCache().getArkivJournalpost(journalpostId);
			return ArkivJournalpostMapper.mapJournalpost(arkivJournalpost, safRequestContext.getRequestCache());
		}).filter(Objects::nonNull);
	}
}
