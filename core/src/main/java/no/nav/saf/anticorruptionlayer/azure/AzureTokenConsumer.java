...
public class AzureTokenConsumer {
	...
	@Cacheable(LokalCacheConfig.AZURE_CLIENT_CREDENTIAL_TOKEN_CACHE)
	public TokenResponse getClientCredentialToken() {
		try {
			...
			throw new RuntimeException(String.format("Klarte ikke hente token fra Azure. Feilet med httpstatus=%s. Feilmelding=%s", e.getStatusCode(), e.getMessage()), e);
		}
	}
