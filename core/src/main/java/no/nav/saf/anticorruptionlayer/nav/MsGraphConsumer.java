...
public class MsGraphConsumer {
	...
	private final UncachedMsGraphConsumer uncachedMsGraphConsumer;
	...
	public MsGraphConsumer(UncachedMsGraphConsumer uncachedMsGraphConsumer, StringRedisTemplate stringRedisTemplate,
						   ValkeyCacheConfig.ValkeyGrupperMedlemskapCacheConfiguration valkeyGrupperMedlemskapCacheConfiguration) {
		this.uncachedMsGraphConsumer = uncachedMsGraphConsumer;
	...
	public boolean isUserInGroup(String navIdent, UUID azureAdGroup) {
		...
		if (!stringRedisTemplate.hasKey(valkeyKeyForIdent)) {
			Optional<User> user = uncachedMsGraphConsumer.getUser(navIdent);
			Set<String> groups = uncachedMsGraphConsumer.getRelevantGroupsForUser(user);
