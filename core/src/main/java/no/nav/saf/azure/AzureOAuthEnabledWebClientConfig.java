...
public class AzureOAuthEnabledWebClientConfig {
	...
	@Bean
	List<ClientRegistration> clientRegistration(AzureProperties azureProperties, SafProperties safProperties) {
		return List.of(
				ClientRegistration.withRegistrationId(CLIENT_REGISTRATION_DOKARKIV)
						...
						.scope(safProperties.getEndpoints().getDokarkiv().getScope())
						.build(),
				...
				ClientRegistration.withRegistrationId(CLIENT_REGISTRATION_PENSJON)
						...
						.build(),
				ClientRegistration.withRegistrationId(CLIENT_REGISTRATION_FPSAK)
						.tokenUri(azureProperties.openidConfigTokenEndpoint())
						.clientId(azureProperties.appClientId())
						.clientSecret(azureProperties.appClientSecret())
						.clientAuthenticationMethod(CLIENT_SECRET_BASIC)
						.authorizationGrantType(CLIENT_CREDENTIALS)
						.scope(safProperties.getEndpoints().getFpsak().getScope())
						.build(),
				ClientRegistration.withRegistrationId(CLIENT_REGISTRATION_K9_SAK)
						.tokenUri(azureProperties.openidConfigTokenEndpoint())
						.clientId(azureProperties.appClientId())
						.clientSecret(azureProperties.appClientSecret())
						.clientAuthenticationMethod(CLIENT_SECRET_BASIC)
						.authorizationGrantType(CLIENT_CREDENTIALS)
						.scope(safProperties.getEndpoints().getK9sak().getScope())
						.build()
		);
	}
}
